...
- Requires `bash`, `jq`, `awk`, `git`, `date`. All present on a default macOS or Linux install once `jq` is added. The `mr` segment additionally needs [`glab`](https://gitlab.com/gitlab-org/cli) for GitLab remotes or [`gh`](https://cli.github.com) for GitHub remotes, and is skipped without the matching CLI.
